...
function validateNumber(str){
    let pureNum = str.replace(/-/g, '')
    
    for (let i =0; i<pureNum.length; i++){
      
      if ((pureNum[0] === '0' && pureNum[1] === '7')&& pureNum.slice(2).length === 9){
        return 'In with a chance'
      } else if ((pureNum[0] === '+' && pureNum[1] === '4'&& pureNum[2] === '4'&& pureNum[3] === '7')&& pureNum.slice(4).length === 9){
        return 'In with a chance'
      } 
