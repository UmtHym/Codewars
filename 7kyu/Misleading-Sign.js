...
function gaslighting(shirtWord, yourWord, friendsLetters) {
    ...
    if (shirtWord === yourWord || 
      (shirtWord.includes(friendsLetters) && yourWord.includes(friendsLetters) && friendsLetters.length === shirtWord.length)) {
    return false;
  }
    for (const letter of friendsLetters) { 
      if (shirtWord.indexOf(letter) !== yourWord.indexOf(letter)) {
        return true; 
      }
    }
    return false;
  ...
  // Check if the words are the same, meaning no attempt at fooling 
  // Loop through each letter in friendsLetters
  // Check if the letters are at different positions in the words
  // Friend can detect the difference
  // If the loop finishes without finding a difference, the friend cannot tell